...
class TestGroupConsecutiveSegments:
    ...
    def test_different_tones_still_merged_with_shift(self):
        """Consecutive lines from same character with different tones should merge,
        with a combined tone description."""
        segs = [
            ScriptSegment(type=SegmentType.NARRATION, character="narrator",
    ...
    def test_mixed_scenario(self):
        """Complex scenario: narrator group, sfx, dialogue, narrator group."""
        segs = [
            ScriptSegment(type=SegmentType.NARRATION, character="narrator",
        ...
        result = _group_consecutive_segments(segs)
        assert len(result) == 4
        # First two narrator lines merged
    ...
    def test_none_tone_segments_merged(self):
        """Segments with no tone should merge and result in None tone."""
        segs = [
            ScriptSegment(type=SegmentType.NARRATION, character="narrator",
