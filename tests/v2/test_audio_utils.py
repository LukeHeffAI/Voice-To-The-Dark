"""Unit tests for services.audio_utils."""
...
class TestCleanupTempFiles:
    ...
    def test_unremovable_file_logs_warning(self, tmp_path):
        ...
        with patch("services.audio_utils.os.remove", side_effect=PermissionError("denied")):
            with patch("services.audio_utils.logging") as mock_logging:
                cleanup_temp_files(folder=str(tmp_path), days=7)
...
class TestPurgeTempFolder:
    ...
    def test_logs_warning_on_rmtree_failure(self, tmp_path, monkeypatch):
        ...
        with patch("services.audio_utils.shutil.rmtree", side_effect=OSError("boom")):
            with patch("services.audio_utils.logging") as mock_logging:
                purge_temp_folder(folder=target)
                mock_logging.warning.assert_called_once()
...
class TestStitchAudioFiles:
    ...
    @patch("services.audio_utils.AudioSegment")
