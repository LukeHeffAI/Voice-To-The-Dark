...
def _estimate_tts_chars(story) -> int:
    ...
    Uses the v2 ORM relation: story.script (NarrationScript model) with
    related segments, rather than the v1 script_json blob.
    """
    try:
        script = story.script
        voice_types = [SegmentType.NARRATION.value, SegmentType.DIALOGUE.value]
        segments = script.segments.filter(type__in=voice_types)
        total = sum(len(seg.text or "") for seg in segments)
        if total > 0:
            return total
    except Exception:
        pass
