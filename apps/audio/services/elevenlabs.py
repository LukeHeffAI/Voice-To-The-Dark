...
from apps.audio.services.audio_utils import cleanup_temp_files, create_tmp_folder, stitch_audio_files
...
VOICE_PREVIEW_TEXT = (
    "The shadows crept closer, and I knew then that something unspeakable was watching from the darkness."
)
...
def generate_sfx(
    ...
    """
    sfx_cache_dir = str(settings.SFX_CACHE_DIR)
...
def generate_voice_preview(voice_id: str, model: str = MODEL_ELEVEN_V3) -> str:
    ...
    """
    voice_preview_dir = str(settings.VOICE_PREVIEWS_DIR)
    os.makedirs(voice_preview_dir, exist_ok=True)

    cached_path = os.path.join(voice_preview_dir, f"{voice_id}.mp3")
