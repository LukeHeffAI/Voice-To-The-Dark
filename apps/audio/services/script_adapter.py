...
from apps.audio.schemas import CharacterProfile, NarrationScript, ScriptSegment
...
def _adapt_section(
    ...
    if response.stop_reason == "max_tokens":
        logger.warning(
            "Claude response truncated (max_tokens) for '%s'; input section may be too long for a single request",
            title,
        )
    ...
    try:
        ...
        raise ValueError(f"Script generation failed: Claude did not return valid JSON ({exc})") from exc
    ...
    *prior_characters*, when provided, seeds the character map so the first
    ...
    while pending:
        ...
        if iterations > MAX_ITERATIONS:
            raise ValueError(f"Story '{title}' required too many sections to process (exceeded {MAX_ITERATIONS})")
    ...
    final_characters = {k: CharacterProfile(**v) for k, v in combined_characters.items()}
