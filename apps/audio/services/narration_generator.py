...
from apps.audio.schemas import NarrationScript, ScriptSegment, SegmentType
from apps.audio.services.audio_mixer import mix_narration
from apps.audio.services.elevenlabs import generate_audio, generate_sfx
from apps.audio.services.segment_cache import lookup as cache_lookup
...
def generate_narration(
    ...
    """
    if not output_path:
        output_path = str(settings.STORIES_DIR / f"{uuid.uuid4()}.mp3")
    os.makedirs(os.path.dirname(output_path), exist_ok=True)

    segment_cache_dir = str(settings.SEGMENT_CACHE_DIR)
    ...
    for i, segment in enumerate(merged_segments):
        ...
        if cache_result.is_hit and not bust_cache:
            cache_hits += 1
            logger.info(f"Reusing cached segment {i + 1}/{len(merged_segments)}: {segment.type.value}")
            segment_files.append((cache_result.cached_path, segment))
        else:
            cache_misses += 1
            logger.info(f"Generating segment {i + 1}/{len(merged_segments)}: {segment.type.value}")
            path = _generate_segment(segment, voice_map, cache_result.cached_path)
            if path:
...
def _group_consecutive_segments(
    ...
    while i < len(segments):
        ...
        if len(group) == 1:
            ...
            logger.info(
                f"Merged {len(group)} consecutive segments for '{seg.character}' into one ({len(merged_text)} chars)"
            )
