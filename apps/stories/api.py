...
def _auto_submit_series_parts(
    ...
    if candidate_urls:
        for url in Story.objects.filter(reddit_url__in=candidate_urls).values_list("reddit_url", flat=True):
            if url:
                existing_keys.add(_canonical_url_key(url))
    ...
    for part in series_parts:
        ...
        for netloc in netlocs:
            for path in paths:
                url_variants.add(
                    urlunparse((parsed.scheme, netloc, path, parsed.params, parsed.query, parsed.fragment))
                )
...
@router.post("/submit", response=StoryResponse, auth=JWTAuth())
def submit_story(request, payload: StorySubmitRequest):
    ...
    series_json_value = series_parts if series_parts else None
    ...
    if series_parts:
        _auto_submit_series_parts(series_parts, reddit_url, author, series_json_value, part_count)

    return story


@router.post("/submit-manual", response=StoryResponse, auth=JWTAuth())
def submit_story_manual(request, payload: ManualStorySubmitRequest):
    ...
    if series_parts and url:
        _auto_submit_series_parts(series_parts, url, author, series_json_value, part_count)

    return story


# ── Story listing & retrieval ──────────────────────────────────────
...
def top_nosleep_posts(request, timeframe: str = "alltime", limit: int = 50):
    ...
    existing_urls = set(Story.objects.values_list("reddit_url", flat=True))
    for post in posts:
...
@router.post("/{story_id}/hide", auth=JWTAuth())
def hide_story_from_home(request, story_id: int):
    """Hide a story from the user's Recently Viewed list."""
    view = StoryView.objects.filter(user=request.auth, story_id=story_id).first()
    if view:
        view.hidden = True
        view.save()
    return {"ok": True}
...
@router.post("/{story_id}/unhide", auth=JWTAuth())
...
def get_series_parts(request, story_id: int):
    ...
    # Fall back to cached series_json (already a list from JSONField)
    ...
    # Sort parts by post date (oldest first)
    parts.sort(key=lambda p: p.get("created_utc", 0))

    # Build candidate URLs for DB lookup
    candidate_urls = set()
    for part in parts:
    ...
    existing = {}
    if candidate_urls:
        existing = dict(Story.objects.filter(reddit_url__in=candidate_urls).values_list("reddit_url", "id"))
...
@router.get("/folders/list", response=list[FolderResponse], auth=JWTAuth())
def list_folders(request):
    """List all folders for the current user."""
    folders = StoryFolder.objects.filter(user=request.auth).annotate(story_count=Count("memberships")).order_by("name")
    return [FolderResponse(id=f.id, name=f.name, story_count=f.story_count, created_at=f.created_at) for f in folders]


@router.post("/folders/create", response=FolderResponse, auth=JWTAuth())
def create_folder(request, payload: FolderCreateRequest):
    ...
    if StoryFolder.objects.filter(user=request.auth, name__iexact=name).exists():
        raise HttpError(409, "A folder with this name already exists")

    folder = StoryFolder.objects.create(user=request.auth, name=name)
...
def add_story_to_folder(request, folder_id: int, payload: FolderAddStoryRequest):
    ...
    if StoryFolderMembership.objects.filter(folder_id=folder_id, story_id=payload.story_id).exists():
        return {"ok": True, "message": "Story already in folder"}

    StoryFolderMembership.objects.create(folder_id=folder_id, story_id=payload.story_id)
    return {"ok": True}
